...
export default function CommentModal({details, modalOpen, setModalOpen}: CommentModalProps) {
    ...
    async function sendComment() {
        setLoading(true)
        const response = axios.post(`https://main--stellular-fenglisu-e2b691.netlify.app/api/restaurant/${details._id}/comment`, {
        ...
        <div id='modal' className="w-screen bg-white absolute top-0 left-0">
            <div>
                {!loading ?
                    <div>
                     <div>
                    <button onClick={() => setModalOpen(false)} className="border-white border-[1px] px-[30px] py-[10px] rounded-[20px]">
                ...
                <div className='mt-12'>
                    ...
                    <p className='text-center mt-12 font-["roobert-medium"] text-[1.5rem]'>Deja un comentario para {details.name}</p>
                </div>
